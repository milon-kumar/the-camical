        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Sub Categroy Edit</h4>
                            <div class="basic-form">
                                <form action="{{ route('admin.sub-category.update',$subCategory->id) }}" method="post">
                                        ...
                                        <div class="col-sm-10">
                                            <select name="categories_id" class="form-control @error('categories_id') is-invalid @enderror">
                                                ...
                                                @foreach($categories as $category)
                                                    <option {{ $subCategory->categories_id == $category->id ? 'selected' : '' }} value="{{ optional($category)->id }}">{{ optional($category)->en_name }}</option>
                                                @endforeach
                                            </select>
                                            @error('categories_id')
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-4">
                                            <input type="text" name="en_name" value="{{ $subCategory->en_name }}" class="form-control @error('en_name') is-invalid @enderror" placeholder="English Name">
                                            @error('en_name')
                                            <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="col-sm-4">
                                            <input type="text" name="bn_name" value="{{ $subCategory->bn_name }}" class="form-control @error('bn_name') is-invalid @enderror" placeholder="বাংলা নাম">
                                            @error('bn_name')
                                            <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
